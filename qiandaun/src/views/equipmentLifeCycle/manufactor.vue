  ...
  <div class="content-box">
    <el-form inline :model="form" class="demo-form-inline">
      <el-row type="flex" justify="space-between">
        <el-row type="flex">
          <el-form-item label="设备生产厂家名称">
            <el-input v-model="form.manufactorName" placeholder="搜索" size="mini">
    ...
    >
      ...
      <el-form ref="responseFormRef" :model="responseForm" label-width="140px">
        <el-form-item prop="manufactorName" style="margin: 0" label="设备生产厂家全称" :rules="{required: true, message: '请输入设备生产厂家全称'}">
          <el-input v-model="responseForm.manufactorName" placeholder="请输入设备生产厂家全称" size="small" />
        </el-form-item>
...
<script>
import { getList, saveOrUpdateBotManufactor, deleteBotManufactor } from '@/api/equipmentLifeCycle/manufactor'
export default {
  data() {
    return {
      form: {
  ...
  methods: {
    ...
    submitTap() {
      this.$refs.responseFormRef.validate((valid) => {
        if (valid) {
          ...
          saveOrUpdateBotManufactor(this.responseForm).then(() => {
    ...
    deleteAfirm() {
      this.DialogLoading = true
      deleteBotManufactor(this.deleteId).then(res => {
        this.DialogLoading = false

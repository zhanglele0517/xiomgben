<template>
  <div class="content-box">
    <el-form inline :model="form" class="demo-form-inline">
      <el-row type="flex" justify="space-between">
        <el-row type="flex">
          ...
          <el-form-item label="实时数据库链接信息id">
            <el-input v-model="form.sisRtdbld" placeholder="搜索" size="mini">
              <i slot="prefix" class="el-input__icon el-icon-search" />
            </el-input>
          </el-form-item>
          <el-form-item>
    ...
    >
      ...
      <el-form ref="responseFormRef" :model="responseForm" label-width="160px">
        <el-form-item prop="unitName" label="机组名称" :rules="{required: true, message: '请输入机组名称'}">
          <el-input v-model="responseForm.unitName" placeholder="请输入机组名称" size="small" />
        </el-form-item>
        <el-form-item prop="sisRtdbId" style="margin: 0" label="实时数据库链接信息id" :rules="{required: true, message: '请输入实时数据库链接信息id'}">
          <el-input v-model="responseForm.sisRtdbId" placeholder="请输入实时数据库链接信息id" size="small" />
        </el-form-item>
        <el-form-item prop="id" style="margin: 0" />
        <el-form-item prop="remarks" style="margin: 0" />
        <el-form-item prop="createTime" style="margin: 0" />
        <el-form-item prop="createBy" style="margin: 0" />
      </el-form>
...
<script>
import { saveOrupdate, getList, deleteOrupdate } from '@/api/equipmentLifeCycle/crew'
export default {
  data() {
    return {
      form: {
  ...
  methods: {
    ...
    submitTap() {
      this.$refs.responseFormRef.validate((valid) => {
        if (valid) {
          ...
          saveOrupdate(this.responseForm).then(res => {
    ...
    deleteAfirm() {
      this.DialogLoading = true
      deleteOrupdate(this.deleteId).then(res => {
        this.DialogLoading = false
